...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        const inner = `
            <p>${games[i].name}</p>
            <p>Amount pledged: $${games[i].pledged}</p>
...
function filterUnfundedOnly() {
    ...
    const notFundedGames = GAMES_JSON.filter((game) => {
        return game.pledged < game.goal;
    });
    console.log(notFundedGames.length);
...
const notFundedGamesLen = GAMES_JSON.filter((game) => {
    return game.pledged < game.goal;
}).length;
...
let str = `A total of $${totalAmtRaised.toLocaleString()} has been raised for ${fundedGames.length} games. Currently ${notFundedGamesLen} ${notFundedGamesLen > 1 ? "games remain" : "game remains"} unfunded. We need your help to fund these amazing games!`;
let test = "hello";
